"""
SQLAlchemy ORM Models — Configuration Tables
...
class AtsPlatform(Base):
    ...
    automation_level = Column(String(20), default="manual")  # 'full' or 'manual'
...
class JobSite(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "category IN ('System integrator', 'Consulting firm', 'Staffing vendor', 'Product Company')"
        ),
    )
...
class SiteSelector(Base):
    ...
    __table_args__ = (CheckConstraint("type IN ('listing', 'application')"),)
...
class JobListing(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "status IN ('discovered', 'ready_to_apply', 'applied', 'failed', 'blacklisted')"
        ),
    )
