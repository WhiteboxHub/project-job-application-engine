...
class InfosysStrategy(BaseStrategy):
    ...
    def _get_selectors(self, key, fallback=None):
        ...
        # 2. Try pre-loaded self.selectors (raw DuckDB)
        if self.selectors:
            for sel_type in ['application', 'listing']:
                config = self.selectors.get(sel_type, {})
                if key in config:
                    val = config[key]
    ...
    def find_and_apply_jobs(self):
        ...
        # Get all keywords
        db_keywords = self._get_keywords("search_keywords")
        if db_keywords:
            keywords = db_keywords
            logger.info("Using search keywords from database")
        else:
            search = self.config_data.get("search", {})
            keywords = search.get("keywords", [])
            if not keywords:
                keywords = [search.get("keyword", "AI Engineer")] # Updated default
            logger.info("Using search keywords from JSON/defaults")
        ...
        try:
            ...
            for (u, title) in urls:
                if u not in seen_urls:
                    ...
                    if job_id and self._is_already_applied(job_id):
                        logger.info(f"≡ƒô▒ skipping {job_id} (already applied)")
                        continue
                        
                    seen_urls.add(u)
                    all_job_listings.append({"job_url": u, "job_title": title})
        except Exception as e:
    ...
    def _search_jobs(self, keyword, location, distance):
        ...
        # Try to type keyword for visual feedback (Human-like interaction)
        try:
            # 1. Reveal search bar (often hidden behind a 'SEARCH JOBS' button)
            reveal_selectors = [
                "//button[contains(text(), 'SEARCH JOBS')]",
                "//a[contains(text(), 'SEARCH JOBS')]",
                ".search-btn",
                "button.js_search_jobs"
            ]
            reveal_btn = self._find_element_safe(reveal_selectors)
            if reveal_btn:
                logger.info("Clicking 'SEARCH JOBS' to reveal search input")
                self.human.click_element(reveal_btn)
                time.sleep(2)

            # 2. Find and fill keyword input
            search_selectors = [
                "input.js_search_cp_jobs",
            ...
            search_input = self._find_element_safe(search_selectors)
            if search_input:
                logger.info(f"Typing keyword: '{keyword}' into search box")
                ...
                logger.warning("Search box not found, falling back to direct URL navigation")
                import urllib.parse
            ...
            logger.warning(f"Interactive search failed: {e}. Falling back to direct URL.")
            import urllib.parse
            keyword_encoded = urllib.parse.quote_plus(keyword)
            search_url = f"{base_url}&keyword={keyword_encoded}"
            self.driver.get(search_url)
            ...
            for page in range(1, max_pages + 1):
                ...
                for link in links:
                    try:
                        ...
                        if not title:
                            # Try to find job-title class inside
                            title_elem = link.find_element(By.CSS_SELECTOR, ".job-title")
                            if title_elem:
                        ...
                        if href and href not in seen:
                            if self._is_target_job(title):
                                ...
                                logger.info(f"Γ£û Filtered out: {title}")
                                job_id = href.split("/")[-1]
        ...
        if location_filters:
            ...
            for loc in location_filters:
                ...
                if re.search(pattern, t):
                    logger.info(f"Γ£û Skipping non-USA job (location match '{loc}'): {title}")
                    return False
    ...
    def _apply_to_job(self, job_url):
        ...
        if self.config and self.config.get_selectors('apply_button'):
            logger.info("Using Apply button selectors from database")
        elif self.selectors and 'application' in self.selectors and 'apply_button' in self.selectors['application']:
            logger.info("Using Apply button selectors from pre-loaded database selectors")
        else:
            logger.info("Using hardcoded Apply button selectors (database empty or missing key)")
    ...
    def _enterprise_fill(self, keywords, value):
        ...
        if best_candidate and best_score > 0:
            ...
            # Require higher confidence for select elements to avoid mis-matching
            if tag == "select" and best_score < 2:
                logger.debug(f"Skipping low-confidence select match (score {best_score}) for keywords {keywords}")
                return False
    ...
    def _handle_dynamic_application_sections(self):
        ...
        for step_idx in range(25):
            ...
            if self._page_has_any_field(eeo_keywords):
                ...
                # Scroll to top and screenshot for debugging
                try:
                    self.driver.execute_script("window.scrollTo(0, 0);")
                    import datetime
                    ts = datetime.datetime.now().strftime("%H%M%S")
                    self.driver.save_screenshot(f"logs/eeo_top_{ts}.png")
                    logger.info(f"📸 EEO top screenshot: logs/eeo_top_{ts}.png")
                except Exception:
                    pass
                applicant = self.config_data.get("applicant", {})
                full_name = f"{applicant.get('first_name', '')} {applicant.get('last_name', '')}"
                
                # Log all select names on this page so we can identify unknown selectors
                try:
                    sel_names = self.driver.execute_script(
                        "return Array.from(document.querySelectorAll('select')).map(s => s.name || s.id || '(no-name)').join(', ');"
                    )
                    logger.info(f"📋 Selects on page: {sel_names}")
                except Exception:
                    pass
                ...
                vet_val = applicant.get("veteran") or "No"
                if "no" in vet_val.lower():
                    vet_val = "No, I am not a veteran under one of the classifications listed above"
                ...
                eeo_data = [
                    ...
                    # Removing veteran and employed from enterprise_fill to prevent double-click / toggle race conditions
                    # (self._get_keywords("veteran", ["veteran", "military", "protected"]), vet_val),
                ...
                if self._fill_race_dropdown(target_race=race_val):
                    did_something = True
                ...
                db_selectors = self.config_data.get("selectors", {}).get("application", {}).get("eeo", {})
                
                # Fetch deeper from ConfigManager if available
                # (Overriding the local db_selectors with flattened DB keys if they exist)
                db_vet_no = self._get_selectors("eeo_veteran_no")
                db_contract_no = self._get_selectors("eeo_contract_restriction_no")
                db_dis_no = self._get_selectors("eeo_disability_no")
                db_consent_yes = self._get_selectors("eeo_consent_yes")


                # ---- DIRECT: Contract Restriction ----
                _contract_selectors = db_contract_no or [db_selectors.get("contract_restriction"), "input[type='radio'][name='custom[other][contract_restriction]'][value='No']"]
                _contract_selectors = [s for s in _contract_selectors if s]
                if self._click_any(_contract_selectors):
                    logger.info("✅ Clicked contract restriction")
                    did_something = True
                else:
                    # Fallback heuristic
                    if self._enterprise_fill(
                        self._get_keywords("contractual_restrictions", ["contractual", "non-competition", "non-compete", "restrictive covenant"]),
                        "No"
                    ):
                        did_something = True
                
                # ---- DIRECT: Disability Status ----
                _dis_selectors = db_dis_no or [
                    "#disability_status_false", 
                    "input[type='radio'][name='custom[eeo][disability]'][value='No']"
                ]
                if self._click_any(_dis_selectors):
                    logger.info("✅ Clicked Disability status (No)")

                # ---- DIRECT: "How did you hear about us?" ----
                _source_filled = False
                _src_css_list = []
                if db_selectors.get("source_dropdown"):
                    _src_css_list.append(db_selectors.get("source_dropdown"))
                # Fallback to defaults only if DB is empty
                else:
                    _src_css_list.extend(["select[name='custom[other][reference]']", "select[name='custom[source]']", "select[name*='source']"])

                for _src_css in _src_css_list:
                    try:
                        _src_elems = self.driver.find_elements(By.CSS_SELECTOR, _src_css)
                        for _src_el in _src_elems:
                            self.driver.execute_script("arguments[0].scrollIntoView({block: 'center'});", _src_el)
                            time.sleep(0.5)
                            if not _src_el.is_displayed():
                                continue
                            from selenium.webdriver.support.ui import Select as _Sel
                            _src_obj = _Sel(_src_el)
                            _src_avail = [o.text.strip() for o in _src_obj.options]
                            for _pref in ["Infosys Careers Site", "Other", "LinkedIn", "Indeed"]:
                                if _pref in _src_avail:
                                    _src_obj.select_by_visible_text(_pref)
                                    logger.info(f"✅ 'How did you hear' set to '{_pref}' via {_src_css}")
                                    did_something = True
                                    _source_filled = True
                                    break
                            if _source_filled:
                        ...
                        logger.debug(f"source fill error: {_se}")
                    if _source_filled:
                        break

                # ---- DIRECT: EEO consent radio "Yes" ----
                _eeo_consent_filled = False
                
                # Try DB selector first Priority
                _consent_selectors = [
                    # Precise XPath: clicks the label whose input has name=custom[other][statement] value=Accept
                    "//label[.//input[@name='custom[other][statement]'][@value='Accept']]",
                ]
                if db_consent_yes:
                    _consent_selectors.extend(db_consent_yes)
                
                _consent_selectors.extend([
                    db_selectors.get("consent_yes"),
                    '//*[@id="form_application"]/div[6]/div/div/div[3]/label[1]',
                    '//label[contains(normalize-space(.), "Yes")][ancestor::div[contains(., "accept the above statement")]]',
                    '//div[contains(., "accept the above statement")]//label[1]',
                    '//div[contains(., "accept the above statement")]//input[@type="radio"][1]',
                ])
                _consent_selectors = [s for s in _consent_selectors if s]

                
                if self._click_any(_consent_selectors):
                    logger.info("✅ Clicked EEO consent radio")
                    did_something = True
                    _eeo_consent_filled = True
                if not _eeo_consent_filled:
                    # JS fallback: find any radio near "accept the above statement"
                    try:
                        clicked = self.driver.execute_script("""
                            var containers = document.querySelectorAll('*');
                            for (var c of containers) {
                                if (c.textContent.includes('accept the above statement')) {
                        ...
                        """)
                        if clicked:
                            logger.info("✅ EEO consent radio clicked via JS fallback")
                            did_something = True
                    except Exception as _je:
                        logger.debug(f"EEO consent JS fallback error: {_je}")

                # ---- FINAL STEP: Veteran + Employment JS ----
                # Run LAST — after ALL other EEO interactions — so no prior click can reset them.
                time.sleep(0.3)
                try:
                    js_vet = """
                        // Use name+value to avoid duplicate-ID confusion with consent radio.
                        var el = document.querySelector('input[name="custom[eeo][veteran]"][value="No"]');
                        if (!el) el = document.querySelector('input[name="custom[eeo][veteran]"]');
                        if (el) {
                            el.checked = true;
                    ...
                    """
                    if self.driver.execute_script(js_vet):
                        logger.info("✅ [Final] Veteran status (No) set via JS")
                    else:
                        logger.warning("⚠️ veteran_status_false not found in final JS check")
                except Exception as e:
                    logger.warning(f"Veteran final JS failed: {e}")

                try:
                    js_emp = """
                        // Use name+value to avoid any duplicate-ID issues.
                        var el = document.querySelector('input[name="custom[other][employed]"][value="No"]');
                        if (!el) el = document.querySelector('input[name="custom[other][employed]"]');
                        if (el) {
                            el.checked = true;
                            el.dispatchEvent(new Event('change', {bubbles: true}));
                        }
                        return el ? el.checked : false;
                    """
                    if self.driver.execute_script(js_emp):
                        logger.info("✅ [Final] Previous Employment (No) set via JS")
                        did_something = True
                    else:
                        logger.warning("⚠️ employed_true not found in final JS check")
                except Exception as e:
                    logger.warning(f"Employment final JS failed: {e}")
    ...
    # -------------------------
    def _fill_race_dropdown(self, target_race=None):
        """
        Finds the Race Category dropdown on the Infosys EEO page using the
        known selector select[name='custom[eeo][race]'] and selects Asian (or
        whatever target_race is set to).
        """
        from selenium.webdriver.support.ui import Select

        applicant = self.config_data.get("applicant", {})
        race_val = target_race or applicant.get("race") or "I prefer not to answer"

        # Build priority list: target race first, then prefer-not fallback
        if "opt out" in race_val.lower() or "decline" in race_val.lower() or "prefer not" in race_val.lower():
            priority_options = ["I prefer not to answer", "Decline to Self-Identify", "Not Specified"]
        else:
            priority_options = [race_val, "I prefer not to answer"]  # e.g. ["Asian", ...]

        try:
            # --- Direct hit: known Infosys selector (database prioritized) ---
            DIRECT_SELECTORS = self._get_selectors("eeo_race_dropdown", ["select[name='custom[eeo][race]']", "select[name*='[race]']", ".race-container select"])
            for css in DIRECT_SELECTORS:
                elems = self.driver.find_elements(By.CSS_SELECTOR, css)
                for sel_elem in elems:
                    if not sel_elem.is_displayed():
                        continue
                    select_obj = Select(sel_elem)
                    available = [o.text.strip() for o in select_obj.options]
                    logger.info(f"Race dropdown found via '{css}', options: {available}")
                    for opt_text in priority_options:
                        for avail in available:
                            if opt_text.lower() in avail.lower() or avail.lower() in opt_text.lower():
                                ...
                                return True
                    # Last resort: first non-placeholder option
                    for avail in available:
                        if avail.strip() and avail.strip() not in ["", "Choose", "-", "--", "Select", "Select One"]:
                            select_obj.select_by_visible_text(avail)
                            logger.info(f"Race: fallback selected '{avail}'")
                            return True

            # --- Heuristic fallback: scan all selects for race keywords in options ---
            logger.warning("Race dropdown not found via direct selectors — trying heuristic scan")
            ...
            for sel_elem in selects:
                ...
                try:
                    select_obj = Select(sel_elem)
                    available = [o.text.strip() for o in select_obj.options]
                    all_opts_lower = " ".join([o.lower() for o in available])
                    # Only consider dropdowns that have real race category options
                    race_terms = ["asian", "white", "black or african", "hispanic", "native hawaiian", "american indian", "two or more races"]
                    if not any(term in all_opts_lower for term in race_terms):
                        continue
                    logger.info(f"Race dropdown found via heuristic, options: {available}")
                    for opt_text in priority_options:
                        for avail in available:
                            if opt_text.lower() in avail.lower() or avail.lower() in opt_text.lower():
                                select_obj.select_by_visible_text(avail)
                    ...
                    logger.warning(f"Error scanning race select: {e}")
                    continue
